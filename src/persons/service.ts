...
export async function getPersonVazbyService(
  ...
  // 1) lookup osoby v HS
  let matches: Awaited<ReturnType<typeof searchOsoby>>;
  try {
    matches = await searchOsoby(rawJmeno, prijmeni, date);
  } catch (e) {
    if (e instanceof HlidacStatuMissingTokenError) throw e;
    throw new InvalidInputError(
      `Hlídač státu lookup selhal: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
  if (matches.length === 0) {
    throw new InvalidInputError(
      `Osoba ${rawJmeno} ${prijmeni} (${date}) nebyla v Hlídači státu nalezena.`,
    );
  }
  const top = matches[0];

  // 2) detail s udalostmi
  const detail = await fetchOsobaDetail(top.nameId);
  const udalosti = (detail.udalosti ?? []).filter((u) => VAZBA_TYPES.has(u.typ ?? ""));

  // 3) optional filtr active/historical
  const includeHistorical = args.includeHistorical !== false; // default true
  const filtered = includeHistorical ? udalosti : udalosti.filter((u) => !u.datumDo);

  // 4) sort & dedup unique organizations
  ...
  // sort by datumOd desc — nejnovější vazby první
  ordered.sort((a, b) => {
    const ad = a.datumOd ?? "";
    const bd = b.datumOd ?? "";
  ...
  // 5) ARES resolve — pouze pro unikátní firmy, max MAX_UNIQUE_COMPANIES
  ...
  if (resolveIco) {
    // Pošleme paralelně — AresClient má vnitřní rate-limit token bucket.
  ...
  // 6) sestav výstup
  const vazby: PersonVazba[] = ordered.map((u) => {
    const org = (u.organizace ?? "").trim();
